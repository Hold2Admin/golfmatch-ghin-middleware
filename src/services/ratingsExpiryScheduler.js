...
function getRatingsExpiryRunAttempts() {
  const raw = Number(process.env.GHIN_RATINGS_EXPIRY_RUN_ATTEMPTS || 5);
  return Number.isFinite(raw) ? Math.max(1, Math.min(20, Math.floor(raw))) : 5;
}

function getRatingsExpiryRetryBackoffMs(attempt) {
  const base = Number(process.env.GHIN_RATINGS_EXPIRY_RETRY_BACKOFF_MS || 15000);
  const safeBase = Number.isFinite(base) ? Math.max(1000, Math.floor(base)) : 15000;
...
async function runOnce(options = {}) {
  ...
  inFlight = true;
  const maxAttempts = getRatingsExpiryRunAttempts();
  let lastError = null;
  ...
  try {
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      try {
        ...
        await sleep(getRatingsExpiryRetryBackoffMs(attempt));
      }
    }

    const finalMessage = lastError && lastError.message
      ? lastError.message
      : 'ratings_expiry_failed_after_retries';
    logger.error('Scheduled ratings-expiry NULL exhausted retries', {
      trigger,
      attempts: maxAttempts,
      error: finalMessage
    });
